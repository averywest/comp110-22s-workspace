"""Dictionary related utility functions from last exercise."""
...
def threshold(counting: dict[str, int]) -> list[str]:
    """Determining what the most common numbers selected are."""
    final: list[str] = []
    for count in counting:
        if counting[count] > 100:
            final.append(count)
    return final
